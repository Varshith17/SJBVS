...
const StudyCertificate = props => {
  ...
  useEffect(() => {
    if (props.accountsData) {
      updateData(props.accountsData.studyCertificates[0])
    }
  }, [props.accountsData])
  ...
  function handleSubmit() {
    if (amount !== "" && clazz !== "" && studentName !== "" && uid !== "") {
      saveApplicationForm()
    }
  }
  ...
  const saveApplicationForm = () => {
    const data = {
      ...
      uid: uid,
      class_name: clazz,
      issued_date: moment().format("DD/MM/YYYY"),
    }

    post("/add-studyCertificate-fees", data)
      .then(res => {
        if (res.error) {
          setFailure(true)
        } else {
...
export default connect(null, { setBreadcrumbItems })(StudyCertificate)
